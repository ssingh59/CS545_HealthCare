...
const constructorMethod = (app) => {
	var loggingMiddleware = function (req, res, next) {
		...
		if(!user){
		  console.log(new Date().toUTCString(), req.method  ,req.originalUrl, "(Non-Authenticated User)" , user);
		}
		else{
			console.log(new Date().toUTCString(), req.method  ,req.originalUrl, "(Authenticated User)" , user)
		}
		next()
	  }
